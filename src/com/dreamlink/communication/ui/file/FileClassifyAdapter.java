...
public class FileClassifyAdapter extends BaseAdapter implements ILoadImageCallback{
...
//				cacheBitmap = bitmapLoader.loadImage(fileInfo.filePath, fileInfo.type);
				//////////////方法三////////////
				cacheBitmap = bitmapLoader.loadImage(fileInfo.filePath, fileInfo.type, holderItem.iconView, new ILoadImageCallback() {
					@Override
			...
			holderItem.deleteLayout.setOnClickListener(new DeleteOnClickListener(position, file_type));
	...
	private class DeleteOnClickListener implements OnClickListener{
		...
		public void onClick(View v) {
			...
			.setTitle(mContext.getResources().getString(R.string.item_msg,position + 1))
			.setMessage(mContext.getResources().getString(R.string.confirm_msg, path))
	...
	private class DeleteTask extends AsyncTask<Integer, Void, Boolean>{
		...
		protected Boolean doInBackground(Integer... params) {
			...
			case FileInfoManager.TYPE_AUDIO:
				fileInfoManager.deleteFileInMediaStore(DreamConstant.AUDIO_URI, path);
				break;
			case FileInfoManager.TYPE_VIDEO:
				fileInfoManager.deleteFileInMediaStore(DreamConstant.VIDEO_URI, path);
				break;
			case FileInfoManager.TYPE_IMAGE:
				fileInfoManager.deleteFileInMediaStore(DreamConstant.IMAGE_URI, path);
			default:
