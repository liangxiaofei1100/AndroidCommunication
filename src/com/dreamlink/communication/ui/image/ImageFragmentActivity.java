...
public class ImageFragmentActivity extends FragmentActivity implements TabContentFactory, OnTabChangeListener{
	...
	private BroadcastReceiver imageReceiver = new BroadcastReceiver(){
		...
		public void onReceive(Context context, Intent intent) {
			...
			int cameraSize = intent.getIntExtra(Extra.CAMERA_SIZE, mCamearLists.size());
			int gallerySize = intent.getIntExtra(Extra.GALLERY_SIZE, mGalleryLists.size());
	...
	public void initViews(){
		...
		mTabHost.addTab(mTabHost.newTabSpec(TAG_ONE).setIndicator(CAMERA_TITLE).setContent(R.id.picture_frag1));
		mTabHost.addTab(mTabHost.newTabSpec(TAG_TWO).setIndicator(GALLERY_TITLE).setContent(R.id.picture_frag2));
...
//			mImageInfos = mediaScan.getImageThumbnail();
		...
		protected void onPostExecute(Integer result) {
			...
			mTextView2.setText(GALLERY_TITLE + "(" + mGalleryLists.size() +")");
		}
